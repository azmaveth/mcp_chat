defmodule MCPChat.Streaming.EnhancedConsumer do
  ...
  @default_config %{
    buffer_capacity: 100,
    write_interval: 25,    # ms - how often to flush buffer
    min_batch_size: 3,     # minimum chunks to batch
    max_batch_size: 10,    # maximum chunks per write
    slow_consumer_threshold: 50  # ms - when to consider consumer slow
  }
  ...
  def init(opts) do
    ...
    state = %State{
      renderer_pid: self(),  # Can be overridden
      accumulated_response: "",
  ...
  def handle_call({:consume_stream, stream, opts}, _from, state) do
    state = %{state | 
      renderer_pid: Keyword.get(opts, :renderer_pid, self()),
      metrics: init_metrics()
    }
    ...
    result = try do
      ...
      # Final flush
      GenServer.call(self(), :finalize, 5000)
    catch
      kind, reason ->
  ...
  defp flush_buffer(state) do
    ...
    if buffer_size > 0 do
      ...
      state = %{state | 
        buffer: new_buffer,
        accumulated_response: state.accumulated_response <> text
      }
      ...
      if write_duration > state.config.slow_consumer_threshold * 1000 do
  ...
  defp finalize_metrics(metrics) do
    ...
    avg_chunk_size = if metrics.chunks_received > 0 do
    ...
    metrics
    |> Map.put(:duration_ms, duration_us / 1000)
    ...
    |> Map.delete(:total_chunk_size)  # Internal metric
  end
end