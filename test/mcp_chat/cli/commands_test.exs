defmodule MCPChat.CLI.CommandsTest do
  ...
  describe "help command" do
    ...
    test "includes all documented commands" do
      ...
      assert output =~ "/servers"    # MCP commands
      assert output =~ "/backend"    # Config commands
      assert output =~ "/context"    # Context commands
      assert output =~ "/cost"       # Cost tracking
      assert output =~ "/alias"      # Alias management
    end
  end
  ...
  describe "tokens command" do
    ...
    test "sets max tokens" do
      output = capture_io(fn ->
        Commands.handle_command("tokens 2048")
      end)
      
      assert output =~ "Max tokens" or output =~ "2048"
    end
